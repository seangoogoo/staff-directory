...
#### July 28, 2026
*Real 404s, Self-Referencing Canonical, Indexable Title*

- Answered a Google Search Console warning received the same day on the domain property `sc-domain:jensen-siu.net` — "Duplicate page without user-selected canonical". Confirmed from the outside first: `/`, `/index.php` and `/?page=1` each answered `200` with **98 582 identical bytes** and no `<link rel="canonical">` anywhere in the HTML, while URL inspection reported the home page as `Submitted and indexed` with `userCanonical: null` and `googleCanonical` pointing at itself. That is the literal definition of the message
- The serious half was the absence of any 404. `includes/404_handler.php` set `http_response_code(404)` and then immediately `header('Location: … /index.php?not_found=1')` — a `302` whose target answered `200` with 99 138 bytes of the same directory page. Verified on the live site for `/nimportequoi`, `/une/page/inexistante`, `/robots.txt.bak` and `/sitemap.xml`. Google calls that a soft 404: the indexable URL space was **infinite**, every dead link inbound minting one more twin of the home page with no canonical to collapse it
- `404_handler.php` now renders the error page in place — site header, `page_not_found` title, message, a link back to the directory, footer — with `http_response_code(404)` and `X-Robots-Tag: noindex` sent from PHP, deliberately not from an Apache directive. No redirect: an error page must carry the error status itself
- `/index.php?not_found=1` is a 404 too, guarded at the **top of `public/index.php`** rather than in `HomeController`. `.htaccess` only rewrites requests that match no file, so Apache serves `public/index.php` directly and the router never sees it — a guard in the controller would have covered `/?not_found=1` and missed the exact URL Google had crawled. The yellow "you have been redirected" banner is gone, and so is the wording that described the redirect (`page_not_found_message`, en/fr, rewritten; new `back_to_directory` key in both files)
- Added `canonical_url()` to `functions.php` and a self-referencing `<link rel="canonical">` to `includes/header.php`. It is absolute, always `https` (the only exception is a plain-http local host such as `localhost:8000`, since `$_SERVER['HTTPS']` is unset behind a TLS-terminating proxy and must not decide the scheme), and **always drops the query string** — `/`, `/index.php`, `/?page=1`, `/?login=required` and every filtered view are one page, and a trailing `index.php` is folded into its directory
- Setting the global `$page_noindex` swaps that canonical for `<meta name="robots" content="noindex, follow">`, which is what the 404 page does. The two are never emitted together: a `noindex` on a page that also points a canonical elsewhere propagates the `noindex` to the target. `includes/admin_header.php` got the unconditional `noindex, nofollow` it always deserved
- Fixed the indexed placeholder `<title>My company`. The frontend title is now composed as `"<page> – <site name>"` — the page part defaults to `__('staff_directory')`, a page script overrides it through the global `$page_title`, and the site name is dropped when both would be identical. The placeholder came from the seeds: `database/staff_dir.sql` (`'My company'`), `database/staff_dir_clean.sql` (`'My Company'`) and `staff_dir_env/.env_example` (`DEFAULT_FRONTEND_TITLE`) now all ship `Staff Directory`. Code cannot fix a value already stored in a live database, so that row was updated separately (one edit in `/admin/settings.php`, or one `UPDATE app_settings`)
- Removed the config flag that made all of this optional: `routing.redirectUnmatchedToIndex` selected between "redirect unmatched to index" and a bare-text 404. The behavior it selected no longer exists, so the flag, the `Router::$redirectUnmatchedToIndex` property and the constructor parameter that read it are gone — `Router` takes no arguments now. That removed the **last** consumer of `config/app.php`, which the reviews then caught as fully dead code: `load_app_config()` (`functions.php`) and `config/app.php` are deleted, ~90 lines, verified by `grep -rn load_app_config` returning nothing but the definition and `grep -rn 'new Router'` returning only `new Router()`. The `README.md` directory tree lost its `config/app.php` line
- Verified on the Apache vhost (`https://staffdirectory.local`, the same LiteSpeed-less mirror of the production layout): `/`, `/index.php` and `/?page=1` all answer `200` with the single canonical `https://staffdirectory.local/`; `/zzz-inexistant`, `/une/page/inexistante` and `/index.php?not_found=1` answer **404** with `X-Robots-Tag: noindex`, the robots meta, **no** canonical and the title `Page Not Found – Staff Directory`; `?lang=fr` gives `Page non trouvée`. With the database temporarily set to `My company`, the title composed as `Staff Directory – My company` — restored afterwards. `logs/debug.log` shows the `INFO: 404 Not Found` line and no new warning
- Search Console checks run through a small dependency-free client (JWT RS256 signed by `openssl`) copied into `tools/` so this repository never calls a script living in another one. It is **gitignored**, like `AGENTS.md` and `documentation-private/`: it is tooling specific to this installation, useless to anyone cloning the app, and it points at a service-account private key. That key lives outside the repository directory entirely; `.gitignore` also catches a stray copy dropped at the root, and the deployment command excludes the client — `.gitignore` stops a commit, never a file transfer. Host-specific details stay in the private ops notes
- Out of scope on purpose: no `sitemap.xml`, no origin `robots.txt` — Cloudflare "Managed robots.txt" answers that URL entirely (200, `User-agent: * / Allow: /` plus AI-crawler blocks, no origin content appended), so the origin's response for it is irrelevant
- Four reviews then ran over the whole session — security, correctness, over-engineering, documentation — and five findings were applied:
  - **The client script was untracked but not gitignored**, so the next `git add -A` would have published the service-account address and its GCP project name in a **public** repository, against this project's own rule that no account name belongs in it (`git grep` confirmed those strings appear in no tracked file today). It is gitignored now, and the identity is no longer written in the file at all: it is read from the key at call time (`jq -r .client_email`)
  - **The canonical host no longer has to come from the client.** `canonical_url()` read `$_SERVER['HTTP_HOST']`; called in isolation with `HTTP_HOST=evil.example` it did return `https://evil.example/`. Not a live hole — Apache answers `421` to a mismatched `Host` on the vhost, Cloudflare only forwards the configured host, the HTML is not edge-cached, and the value is filtered plus `htmlspecialchars()`-escaped — but a latent one, and it is the tag that steers indexing. A new optional `.env` key, `CANONICAL_HOST` (empty by default, so the app still works on any domain out of the box), pins the host when set
  - **`$GLOBALS['page_title']` / `$GLOBALS['page_noindex']` became `global $page_title` / `global $page_noindex`.** Same mechanism, but `$GLOBALS[` appeared nowhere else in the codebase while `global $conn`, `global $logger` and `global $app_settings` are the established idiom — including in the two files concerned. `header.php` composes into a local (`$head_title`) so it never writes back into the global
  - **`front-controller.php`'s docblock still read "It preserves the behavior of redirecting unexisting requests to index.php"** — untouched by the diff, and made false by it. Rewritten
  - The `canonical_url()` docblock cited `/admin/index.php -> /admin/`, a path no canonical is ever built for (admin pages go through `admin_header.php`, which emits none). Trimmed to the real case
- Findings rejected on purpose, with the reason: the OAuth token and the signed JWT transit through `curl`'s argv, so they are visible in `ps` — single-user workstation, one-hour token, and the private key itself is *not* exposed (the `<(jq …)` process substitution passes it through `/dev/fd`, never a temp file or argv); `//index.php` folds to `//` — cosmetic, and unreachable since that path matches no route and therefore 404s without a canonical; two 2025 devbook entries still say "a dedicated 404 handler with proper redirection" — dated history describing the state of the time, not a claim about today
- Also measured during the reviews, worth recording: a **subdirectory** install keeps its prefix (`/staff-directory/index.php` → `https://host/staff-directory/`, never `/`, because the `#(^|/)index\.php$#` replacement keeps the captured slash); no double escaping in the title (`admin/settings.php` writes through `sanitize_input($value, false)`, i.e. raw, and `header.php` escapes once); i18n parity at 84/84 identical keys in `languages/{en,fr}/common.php`; and `ErrorDocument 404 /front-controller.php` stays inert on a subdirectory install because the rewrite catches every non-file request first — pre-existing, not aggravated

#### July 28, 2026
*Admin Password Stored Only as a Hash*

- Finished what the previous entry started: `staff_dir_env/.env` no longer holds the admin password in readable form anywhere, and the cleartext code path is gone. `config/auth_config.php` defines `ADMIN_PASSWORD_HASH` only — `ADMIN_PASSWORD` is not read at all — and `verify_login()` is a single `password_verify()` against it
- Fixed the write bug that made the whole migration impossible first. `update_env_file()` (`install.php`) updated keys with `preg_replace('/^KEY=.*$/m', $key . '=' . $value, $content)`, passing the value as a **replacement string**: a bcrypt hash starts with `$2y$`, which `preg_replace` reads as backreferences, so the hash was written corrupted and `password_verify()` failed against it. Measured before the fix: a hash round-tripped through that function came back invalid, while `config/env_loader.php` read it back intact — the corruption was on write. Now `preg_replace_callback()` with the line built inside the callback, and `preg_quote()` on the interpolated key
- `public/install.php` hashes `admin_pass` with `password_hash($password, PASSWORD_DEFAULT)` before writing `.env` (the installer does not load `auth_config.php`, so `PASSWORD_ALGO` is unavailable there), unsets the cleartext key, and passes `['ADMIN_PASSWORD']` to the new `$remove_keys` parameter so a stale cleartext line from an earlier install is replaced by a marker comment carrying no value
- The installer's success message was **never displayed**: `$installed = is_installed()` is re-evaluated after the `.env` write, so a successful install flipped the template to the "already installed" branch and swallowed the `$message` block sitting in the `else`. Hoisted that block above the branch — `installation_completed` now shows, together with a new notice (`admin_password_hashed_notice`, en/fr) warning that the password is no longer recoverable from `.env`
- Added `tools/hash_admin_password.php` for installations that predate the change: it hashes the cleartext `ADMIN_PASSWORD` already in `.env` in place, is idempotent, offers `--dry-run` and `--force`, never prints the password, writes a timestamped backup first (`chmod 0600`), and re-reads the file afterwards to `password_verify()` the stored hash — restoring the backup if that fails, and naming the file to copy back by hand if even the restore fails, so a corrupted write can never lock the single admin account out unnoticed. It refuses to run outside the CLI
- Review caught a lockout path in that tool: it read the cleartext with its own line scanner, which took the **first** `ADMIN_PASSWORD=` line, while `config/env_loader.php` lets the **last** occurrence win. On a `.env` with the key twice — a plausible hand edit — it hashed a password the application had never used and left the second line live, so the admin was locked out with a hash that verified nothing and a cleartext password still in the file. Reproduced, then fixed by reading the values through `load_env()` itself (single source of truth for quoting and precedence) and by replacing **every** active cleartext line, not just the first
- `admin_password_configured()` also rejects a value `password_hash()` could not have produced. Measured on both 7.4.33 and 8.4.23: `password_get_info()` returns algo `'2y'` for a complete bcrypt hash and `null` for anything else — truncated at 12 characters or at 59 alike. A hash cut short by a botched FTP upload used to look configured and reject every password; it now yields the dedicated diagnostic message instead of a credentials error
- A login attempt against an empty `ADMIN_PASSWORD_HASH` now returns a dedicated message (`admin_password_not_configured`, en/fr) instead of "Invalid username or password", and logs an ERROR: with no reset mechanism and no second account, a misconfigured `.env` is otherwise indistinguishable from a typo. Both login endpoints — `admin/auth/login.php`, which Apache and `php -S` serve directly, and `AuthController@login` behind the route — were updated, and their generic failure message now goes through `__('login_failed')` instead of a hardcoded English string
- Verified: `.env` migrated on this installation (`grep '^ADMIN_PASSWORD='` returns nothing) and login still succeeds with the usual password, wrong password rejected; a full install on a throwaway database (`staff_dir_b2test`) with a password deliberately containing `$` and `\` wrote a valid hash, no cleartext anywhere in the file, displayed both messages, and its admin logged in — `.env` restored bit-identical (md5) after each test, test database dropped
- `staff_dir_env/.env_example` now ships an empty `ADMIN_PASSWORD_HASH=` with generation instructions. It is copied verbatim to `.env` when the file is missing, so an empty value is deliberate: no login is possible, and the login form says exactly that
- `npm run lint:php74` now also covers `tools/` (the `find` list was `public config database`)
